...
export class UDPService {
    ...
    sendUDPMessage(message: string, port: number, addresses: Array<string>, ttl: number, timetolisten: number) {
        ...
        console.log(`Is Chrome defined? ${!!chrome}`);
        console.log(`Is Sockets defined? ${chrome && chrome.sockets}`)
    ...
    closeUDPService() {
        // close the socket
        if (chrome && typeof chrome.sockets !== 'undefined') chrome.sockets.udp.close(this.socketid);
